  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `${window.location.origin}:8000/api/tournaments/create/`,
        {
          method: "POST",
          headers: {
...
async function joinTournament(tournamentId) {
  ...
  try {
    loadingOverlay.show();
    const response = await fetch(
      `${window.location.origin}:8000/api/tournaments/${tournamentId}/join/`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("access")}`,
...
async function startTournament(tournamentId) {
  ...
  try {
    loadingOverlay.show();
    const response = await fetch(
      `${window.location.origin}:8000/api/tournaments/${tournamentId}/start/`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("access")}`,
...
async function loadTournamentBracket(tournamentId) {
  ...
  try {
    loadingOverlay.show();
    const response = await fetch(`${window.location.origin}:8000/api/tournaments/${tournamentId}/bracket/`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("access")}`,
      },
    });
...
async function loadAvailableTournaments() {
  ...
  try {
    loadingOverlay.show();
    const response = await fetch(`${window.location.origin}:8000/api/tournaments/get/`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("access")}`,
      },
    });
