...
function cancelRegistration() {
	...
	messageModal.show(
		...
		if (accept) {
			...
			try {
				const response = await fetch(`http://${window.location.origin}:8000/api/users/delete/`, {
					method: "DELETE",
					headers: {
...
async function two_fa_enable() {
	try {
		...
		const response = await fetch(
			`http://${window.location.origin}:8000/api/users/twofa/enable/`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
...
async function verifyOtpCode() {
	try {
		...
		const response = await fetch(
			`http://${window.location.origin}:8000/api/users/twofa/verify/`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
...
async function requestOtp() {
	...
	try {
		...
		const enableResponse = await fetch(
			`http://${window.location.origin}:8000/api/users/setRecoverOTP/`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
...
async function checkRecoverOTP() {
	...
	try {
		const enableResponse = await fetch(
			`http://${window.location.origin}:8000/api/users/checkRecoverOTP/`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
